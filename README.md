...
| Directory | Holds |
|---|---|
| `components/ui/` | Vendored from the shadcn registry. Do not hand-edit; `shadcn` overwrites it. |
| `components/modules/<domain>/` | The three interactive features (`apply`, `contact`, `diagnostic`), each with `schema.ts`, a service, a component, a `state.ts` and a barrel. |
| `components/shared/` | Cross-cutting components, consumed through its `index.ts` barrel. |
| `components/marketing/` | Presentational sections that take typed props. |
| `components/animations/` | Motion primitives, adapted from React Bits. |
| `components/brand/` | The logo mark. |

A module never reaches into another module's internals; it consumes the
barrel. Shared code lives in `components/shared/`, not loose at the root.
