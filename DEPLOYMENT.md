...
| `RESEND_FROM_EMAIL` | e.g. `coh0rt@mrvin100.de` | Must be on a domain verified in Resend. |
| `APPLICATION_NOTIFICATION_EMAIL` | where applications land | Defaults to the public contact address if unset. |
...
3. Add it to `lib/seo.ts` inside `buildMetadata`:
...
4. Set `GOOGLE_SITE_VERIFICATION` in Vercel and redeploy.
...
such as closing early because all 12 seats filled.
...
{ id: "26a", statusOverride: "completed", ... },
{
  ...
  start: "...", end: "...",
  seats: { min: 8, max: 12 },
  tuition: 4_500_000,       // standard rate; the founding rate was 26A only
  standardTuition: 4_500_000,
  instalment: { count: 2, amount: 2_375_000 },
  founding: false,
},
```
...
`lib/constants.ts` → `channels`. Empty strings are treated as "not
published yet" and the section falls back to a short note plus the contact link,
so no dead links ship. Fill in `whatsappCommunity`, `discord` and `facebook`
when the URLs exist.
